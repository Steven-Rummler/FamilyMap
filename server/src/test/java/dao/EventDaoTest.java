...
public class EventDaoTest {
    ...
    public void findFail() throws DataAccessException {
        // Attempts to find an Event that is not in the database
        assertEquals(null, eDao.find("invalidID"));
    }
    ...
    public void clearUserPass() throws DataAccessException {
        ...
        assertEquals(null, eDao.find(bestEvent.getPersonID()));
    }

    @Test
    public void clearUserPass_MultipleUsers() throws DataAccessException {
        eDao.insert(bestEvent);
        String originalid = bestEvent.getEventID();
        bestEvent.setEventID("anotherid");
        eDao.insert(bestEvent);
        eDao.clearUser(bestEvent.getAssociatedUsername());
        assertEquals(null, eDao.find(originalid));
        assertEquals(null, eDao.find(bestEvent.getPersonID()));
    }
    ...
    public void personsForUserPass() throws DataAccessException {
        eDao.insert(bestEvent);
        assertEquals(bestEvent, eDao.eventsForUser(bestEvent.getAssociatedUsername()).get(0));
    }

    @Test
    public void personsForUserPass_NoPersons() throws DataAccessException {
        eDao.insert(bestEvent);
        assertEquals(0, eDao.eventsForUser("fakeuser").size());
    }
}
